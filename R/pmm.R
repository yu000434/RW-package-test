pmm_component <- function(model) {
  # Stored PMM score and influence contribution used in the RW variance.
  list(S_mis_imp = model$pmm_score, d = model$pmm_d)
}

#' Extract PMM donor IDs
#'
...
topk_probability <- function(x_obs, x_mis, beta_hat, beta_dot, covariance, donors) {
  n_obs <- nrow(x_obs)
  n_par <- ncol(x_obs)
  
  # PMM matching predictions: observed donors use beta_hat,
  # while missing recipients use the posterior draw beta_dot.
  ...
  # Sort donor predictions and construct the interval in which each donor
  # belongs to the recipient's top-k nearest donor set.
  ...
  # Probability that each donor belongs to the top-k set, together with
  # its derivative with respect to the matching model coefficients.
  ...
  # Probabilities of entering the top-k donor pool must sum to k;
  # their derivatives must sum to zero because this total is fixed.
...
pmm_inputs <- function(imps, variable, p) {
  ...
  # Reconstruct the covariance of the PMM regression draw and use it
  # to obtain probabilities of entering the top-k donor pool and their derivatives.
  fit <- utils::getFromNamespace("estimice", "mice")(
    x_obs, data[[variable]][observed], ridge = model$setup$ridge
  )
  matching <- topk_probability(
    x_obs, x_mis, drop(model$beta.hat), drop(model$beta.dot),
    model$sigma.dot^2 * fit$v, model$setup$donors
  )
  
  list(missing = !observed, matching = matching)
}
...
pmm_columns <- function(imps, variable) {
  ...
  # Number of estimating equation parameters contributed by each
  # imputation model
...
#' @param variable Name of the PMM-imputed variable.
#' @param expected_score Function for computing expected analysis scores.
...
pmm_kappa <- function(object, variable, expected_score, ...) {
  ...
  for (p in seq_len(object$m)) {
    ...
    # Expected analysis-score contribution
    score <- expected_score(
      object, variable, p, inputs$missing,
      inputs$matching$donor_prediction, ...
    )
    
    expected_dim <- dim(inputs$matching$probability)
    ...
    # Cross term: combine the analysis score with the derivative of the
    # donor-selection probability. Divide by k because selection is uniform
    # among the k donors in the top-k set.
    for (j in seq_along(pmm_names)) {
      derivative <- inputs$matching$derivative[[j]] / donors
      kappa[, j] <- kappa[, j] +
...
pmm_score_lm <- function(object, variable, p, missing, donor_mean) {
  ...
  if (!inherits(model, "lm") || inherits(model, "glm") ||
      !identical(stats::formula(model)[[2L]], as.name(variable))) {
    stop("Automatic PMM pooling requires the untransformed PMM variable as the lm() response.")
  }
...
pmm_score_binomial <- function(object, variable, p, missing, donor_mean,
  ...
  x <- imputation_design(object$mids$data, downstream)
  downstream_base <- drop(x[missing, base_names, drop = FALSE] %*% beta[base_names])
  observed_response <- object$mids$data[[response]][missing]
  
  if (is.factor(observed_response)) {
    observed_response <- as.integer(observed_response) - 1L
  }
  
  # Integrate over the donor's predictive distribution
  ...
  upper <- 10
  half_width <- pmax((upper - lower) / 2, 0)
  ...
  # Compute the expected binomial analysis score for each possible donor,
  # integrating over uncertainty in the continuous PMM value.
  for (q in seq_len(quadrature_order)) {
    value <- donor_value[, q]
    
    analysis_mean <- stats::plogis(
      stats::coef(model)[[1L]] +
        stats::coef(model)[[variable]] * value
    )
    
    downstream_mean <- stats::plogis(
      outer(value * beta[[variable]], downstream_base, "+")
    )
    
    residual <- sweep(downstream_mean, 1L, analysis_mean, "-")
    
    # If the binary response is observed, use Y - mu directly;
    # otherwise use its conditional expectation under the downstream model.
...
#' @param threshold Lower threshold for including the PMM predictor.
...
pmm_kappa_binomial <- function(object, variable, threshold = -Inf,
  ...
  pmm_kappa(
    object, variable, pmm_score_binomial,
    threshold = threshold,
